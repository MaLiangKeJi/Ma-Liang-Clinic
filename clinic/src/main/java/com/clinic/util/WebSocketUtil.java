...
import com.clinic.dto.vo.WebSocket;
 ...
 */
@Component
@ServerEndpoint(value = "/clinic/openDrug/{userId}")
@Slf4j
public class WebSocketUtil {
     ...
     * user 线程安全的
     */
    private static final Map<Long, WebSocket> userMap = new ConcurrentHashMap<Long, WebSocket>();
...
//        log.info("当前在线人数: " + loginCount);
     ...
     **/
    @OnClose
    public void onClose(@PathParam("userId") Long userId, Session session, CloseReason closeReason) {
        boolean containsKey = userMap.containsKey(userId);
        if (containsKey) {
            // 删除map中用户
